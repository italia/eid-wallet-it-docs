...
**Steps 9-11:** The Mobile Relying Party Instance:
  ...
  2. Generates the :ref:`Relying Party Key Binding Request` in the form of a JWT. This JWT includes ``integrity_assertion``, ``hardware_signature``, ``nonce``, ``hardware_key_tag``, and ``cnf`` (representing ``key_pub``); it is signed using ``key_priv``.
