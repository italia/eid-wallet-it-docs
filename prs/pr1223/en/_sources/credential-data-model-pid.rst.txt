...
.. list-table::
    ...
    * - **nationalities**
      ...
      - Commission Implementing Regulation `EU_2024/2977`_
    * - **picture**
      - OPTIONAL until the expiry of the 24-month transitional period following the entry into force of Regulation `EU_2024/2977`_. After that period has elapsed, this attribute is REQUIRED. *String*. Facial image encoded as a data URL containing the base64-encoded JPEG portrait, compliant with the quality requirements for a full frontal image type as set out in ISO/IEC 39794-5 or, for backward compatibility, ISO/IEC 19794-5, clauses 8.2, 8.3 and 8.4. It MUST NOT include the headers or blocks specified in clause 5 of ISO/IEC 19794-5, except for the image data itself. Except where the User explicitly opts out, where applicable; in case of opt-out, the value MUST be empty, as specified in ARF HLR **PID_03**.
...
.. list-table::
    ...
    * - **nationality**
      ...
      - ``eu.europa.ec.eudi.pid.1``
    * - **portrait**
      - OPTIONAL until the expiry of the 24-month transitional period following the entry into force of Regulation `EU_2024/2977`_. After that period has elapsed, this attribute is REQUIRED. *(bstr)*. Facial image in JPEG format, compliant with the quality requirements for a full frontal image type as set out in ISO/IEC 39794-5 or, for backward compatibility, ISO/IEC 19794-5, clauses 8.2, 8.3 and 8.4. It MUST NOT include the headers or blocks specified in clause 5 of ISO/IEC 19794-5, except for the image data itself. Except where the User explicitly opts out, where applicable; in case of opt-out, the value MUST be empty, as specified in ARF HLR **PID_03**.
      - ``eu.europa.ec.eudi.pid.1``
    * - **personal_administrative_number**
