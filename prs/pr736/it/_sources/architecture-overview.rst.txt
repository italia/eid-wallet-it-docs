...
L'ecosistema IT-Wallet è costruito su un'architettura multi-livello, dove gli organi di governance stabiliscono e mantengono l'infrastruttura della fiducia, gli Attori Primari implementano e gestiscono le soluzioni tecniche, e i sistemi esterni forniscono servizi aggiuntivi.
...
Il livello di governo dell'ecosistema richiede a tutti i suoi partecipanti siano conformi alle specifiche tecniche e ai requisiti di sicurezza.
...
Il livello operativo primario dell'ecosistema implementa il ciclo di vita delle Credenziali Digitali attraverso interazioni coordinate tra soluzioni tecniche specializzate, in modo che tutte le operazioni sulle Credenziali mantengano gli standard di sicurezza e privacy, consentendo al contempo esperienze Utente senza interruzioni.
...
L'architettura consente i seguenti processi di interazione:

  1. **Onboarding e Federazione delle Entità**: Solo entità qualificate e conformi vengono registrate nella Federazione IT-Wallet. Il Trust Registry viene aggiornato durante la registrazione, consentendo ai partecipanti di monitorare lo stato della federazione. Questo processo può includere valutazioni tecniche e di sicurezza.

  2. **Emissione di Credenziali**: I Fornitori di Credenziali si collegano alle Fonti Autentiche tramite API standardizzate sulla Piattaforma Nazionale di Dati Digitali (PDND) per richiedere Attributi dell'Utente verificati. Le Credenziali Digitali si basano su dati autorevoli e aggiornati con adeguata autorizzazione e tracce di audit.

  3. **Archiviazione e Gestione delle Credenziali**: Le Soluzioni IT-Wallet ricevono e gestiscono le Credenziali Digitali sui dispositivi degli Utenti, consentendo a questi di controllare e utilizzare le Credenziali provenienti da più fonti.

  4. **Presentazione e Verifica delle Credenziali**: Gli utenti presentano le Credenziali Digitali alle Relying Party per la verifica. I sistemi di verifica controllano gli attributi attraverso metodi crittografici e controlli di stato sia per uso pubblico che privato.

L'Infrastruttura della Fiducia gestisce l'onboarding e la revoca delle entità, fornisce schemi di Credenziali e consente ai partecipanti di scoprire e verificare le entità autorizzate e il loro stato. L'Infrastruttura consente la convalida delle catene della fiducia, lo scambio di metadati e l'utilizzo di servizi API di tipo RESTful per operazioni di federazione e valutazione della fiducia.
