...
.. list-table::
  ...
  * - **vp_formats_supported**
    - JSON object defining the formats and proof types of Verifiable Presentations and Verifiable Credentials the RP supports. It consists of a list of name/value pairs, where each name uniquely identifies a supported type. The RP MUST support at least ``dc+sd-jwt``. For SD-JWT VC, the value associated with each name/value pair MUST include "**sd-jwt_alg_values_supported**" listing acceptable signing algorithms; for mdoc, include ``alg_values_supported``. The JOSE/COSE headers of presented artifacts MUST match one of the advertised values. See `OpenID4VP`_ §11 and Appendix B.
  * - **jwks**
