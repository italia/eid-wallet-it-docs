...
Il livello di governance richiede a tutti i partecipanti dell'ecosistema di conformarsi agli standard e ai requisiti di sicurezza e tecnici.
...
Il livello operativo primario implementa il ciclo di vita delle Credenziali Digitali attraverso interazioni coordinate tra soluzioni tecniche specializzate, in modo che tutte le operazioni sulle credenziali mantengano gli standard di sicurezza e privacy, consentendo al contempo esperienze utente senza interruzioni.
...
L'architettura consente i seguenti processi di interazione:
  ...
  2. **Emissione di Credenziali**: I Fornitori di Credenziale si collegano alle Fonti Autentiche tramite API standardizzate sulla Piattaforma Nazionale di Dati Digitali per richiedere Attributi dell'Utente verificati. Le Credenziali Digitali si basano su dati autorevoli e aggiornati con adeguata autorizzazione e tracce di audit.

  3. **Archiviazione e Gestione delle Credenziali**: Le Soluzioni IT-Wallet ricevono e gestiscono le Credenziali Digitali sui dispositivi degli utenti, consentendo agli utenti di controllare e utilizzare le credenziali provenienti da più fonti.
...
L'Infrastruttura di Trust gestisce l'onboarding e la revoca delle entità, fornisce schemi di credenziali e consente ai partecipanti di scoprire e verificare le entità autorizzate e il loro stato. Supporta la convalida automatica della catena di fiducia, l'ancoraggio distribuito della fiducia, lo scambio standardizzato di metadati e i servizi API di Federazione per operazioni di federazione sicure e senza interruzioni.
