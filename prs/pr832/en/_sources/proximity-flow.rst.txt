...
- *Device Retrieval* mechanism based on Bluetooth Low Energy (BLE) for the communication sub-phase. *Server Retrieval* mechanism MUST NOT be supported (:ref:`PPR-023 <test-plans-proximity-presentation:PPR-023>`).
...
**Step 16**: After receiving User approval, the Wallet Instance MUST retrieve the requested mdoc Digital Credentials. It then MUST prepare a `SessionData` message containing these Digital Credentials, and it MUST sign the required authentication data (as part of the mdoc authentication process, as specified in [`ISO18013-5`_ #9.1.3]). It MUST encrypt it using the established session keys before transmitting it to the Relying Party Instance over the secure BLE channel. The signing ensures device binding and data integrity. The mdoc response MUST be encoded in CBOR, with its structure outlined in [`ISO18013-5`_ #8.3.2.1.2.2] (:ref:`PPR-006 <test-plans-proximity-presentation:PPR-006>`).
...
.. list-table::
   ...
   * - **Security**
     - *(array)*. Contains two mandatory values:
       ...
       - *(bstr)*. Public ephemeral key generated by the Wallet Instance, used by the Relying Party Instance to derive the Session Key. The key MUST be of a type allowed by the selected cipher suite (:ref:`PPR-022 <test-plans-proximity-presentation:PPR-022>`).

   * - **BleOptions**
     - *(map)*. Provides options for the BLE connection, such as Peripheral Server or Central Client mode, and the device UUID.

       Only `Central Client Mode` MUST be supported by this implementation profile (:ref:`PPR-023 <test-plans-proximity-presentation:PPR-023>`).

   * - **Capabilities**
     - *(map)*. Declares optional capabilities supported by the mdoc, that are:

       - **HandoverSessionEstablishmentSupport** *(bool)*. If present, it MUST be set to `true`. Indicates support for receiving the `SessionEstablishment` message during Negotiated Handover, as defined in [`ISO18013-5`_ #8.2.2.4] (:ref:`PPR-024 <test-plans-proximity-presentation:PPR-024>`).

       - **ReaderAuthAllSupport** *(bool)*. If present, it MUST be set to `true`. Indicates support for receiving the `ReaderAuthAll` structure in the mdoc request, as defined in [`ISO18013-5`_ #8.3.2.1.2.1] (:ref:`PPR-025 <test-plans-proximity-presentation:PPR-025>`).
...
The messages in the mdoc Request MUST be encoded using CBOR. The resulting CBOR byte string for the mdoc Request MUST be encrypted with the Session Key obtained after the Device Engagement phase and MUST be transmitted using the BLE protocol (:ref:`PPR-026 <test-plans-proximity-presentation:PPR-026>`, :ref:`PPR-027 <test-plans-proximity-presentation:PPR-027>`, :ref:`PPR-028 <test-plans-proximity-presentation:PPR-028>`).
...
.. list-table::
   ...
   * - **docRequests**
     - *(array)*. Each entry is a `DocRequest` containing:
       ...
       - **readerAuth** *(COSE_Sign1, CONDITIONAL)*. Used to authenticate the the Relying Party Instance for each `DocRequest`. The signature is computed over `ReaderAuthentication` data, as defined in [`ISO18013-5`_ #9.1.4].
         ...
         This component MUST be present only if `readerAuthAll` is not used (:ref:`PPR-025 <test-plans-proximity-presentation:PPR-025>`).
   ...
   * - **readerAuthAll**
     - *(COSE_Sign1, CONDITIONAL)*. Used to authenticate the Relying Party once for all `DocRequest`s. The signature is computed over `ReaderAuthenticationAll` data, as defined in [`ISO18013-5`_ #9.1.4].
       ...
       This component MUST be present only if `ReaderAuthAllSupport` is set to `true` in the DeviceEngagement structure, and individual `readerAuth` fields are not used (:ref:`PPR-025 <test-plans-proximity-presentation:PPR-025>`).

.. note::
    **Requesting the Wallet Attestation**

    The Relying Party requesting a Wallet Attestation MUST add an object in the **docRequest** array having the ``docType`` set to ``{Trust Anchor reverse domain}.{WalletAttestation}`` as described in :ref:`registry-catalogue:Digital Credentials Catalogue Structure`. The Relying Party MUST NOT include the ``nameSpaces`` parameter in the request (:ref:`PPR-010 <test-plans-proximity-presentation:PPR-010>`).

mdoc Response
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The messages in the mdoc Response MUST be encoded using CBOR and MUST be encrypted with the Session Key obtained after the Device Engagement phase (:ref:`PPR-029 <test-plans-proximity-presentation:PPR-029>`, :ref:`PPR-030 <test-plans-proximity-presentation:PPR-030>`).
...
Each document in **documents** MUST be compliant with the following structure, and it MUST include the following components, unless otherwise specified (:ref:`PPR-029 <test-plans-proximity-presentation:PPR-029>`):
...
If the Wallet Instance and the Relying Party Instance does not send or receive any further requests, the session termination MUST be initiated as follows (:ref:`PPR-007 <test-plans-proximity-presentation:PPR-007>`):
