...
**Steps 22-27**: The Wallet Provider Backend evaluates the Key Attestation Request and MUST perform the following checks (:ref:`WP_143 <wallet-instance-optional-testcases>`):
  ...
  5. The signature of the ``keys_to_attest`` parameter MUST be first validated using the provided ``jwk``. If Key Attestation APIs (OEM) material is included, it MUST be validated according to the device manufacturer's guidelines, as defined in :ref:`wallet-solution-requirements:Use of Key Attestation APIs (OEM)` (:ref:`WP_143h <wallet-instance-optional-testcases>`).
