...
.. list-table::
  ...
  * - CI_196
    - Issuance, Interoperability
    - Minimum grant support
    - Authorization Server metadata advertises at least ``authorization_code``.
  * - CI_197
    - Issuance, Interoperability
    - Credential Offer scheme and grants
    - An ETSI EUDI Wallet offer uses ``eu-eaa-offer://`` and contains at least one final permitted grant object; valid Authorization Code and Pre-Authorized Code offers are accepted.
  * - CI 198
    - Issuance, Trust, Interoperability
    - Signed EUDIW metadata and access certificate
    - The Credential Issuer publishes signed ``application/jwt`` metadata with protected ``x5c`` containing the signing access certificate first and excluding the trust anchor; unsigned EUDIW metadata is rejected.
  * - CI_199
    - Issuance, Trust, Interoperability
    - National metadata path
    - National issuance publishes and consumes final policy-processed Federation metadata only; EUDIW ``x5c`` or WRPRC evidence cannot supplement or authenticate it.
  * - CI_200
    - Issuance, Interoperability
    - ``issuer_info`` signed-payload shape
    - ``issuer_info`` contains the required ``format`` and ``data`` elements.
  * - CI_201
    - Issuance, Security, Privacy
    - PID EDP prohibition
    - Credential Issuer omits ``embedded_disclosure_policy`` for PID configurations.
