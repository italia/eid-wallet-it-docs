...
.. figure:: ../../images/High-Level-Flow-ITWallet-Presentation-ISO-updated.svg
    :figwidth: 100%
    :align: center
    :target: https://www.plantuml.com/plantuml/svg/XLJ1RXCn4BtdAqPxebBBWME5K4iR8GI8a53rmiMn9zbHTyV67c_8tpFhtKYEQCGUebrxvhqtpy-vIsAYvBvRc2oUSx_5k5Y0FiQAZ_2KC899aCjQ-GdQG9Q2OO7csNGT2ZmnIJFM7VujMQ_WCoiDMombmuIVRpuSRk5xG0Pf4MQWK-L90Wr6S4RDxqlB92bh4ok94V8Cs-Dh8E3Zk6FWUVDmNxCMY8GTMacbCKJ_XtgIexvXKk09TfVPZY-cqwtIxuF0DyFEg3b7e1ri1CFDEw04x0K2nfsFFJhbBQobwQ32ymNXLHcocygBna_86CVX6G-0eSLUFpjuGTLeAC8lVjCqoqSSoE8NFBvTHHgQvXhE5_EsQQvEgCCjR5f_A1F_MCE3TmZ4jikEUA-DKz-IzsQlR2nDStuCG_aSReg-ZJLS8qrxXUJ_qc7KlaRZCDRQwyLQUu5LbZvaHKsO4daUyKwfd9wgTXKnc8XW8D6UqO5X1yWs7eECNoAHjWgcMYB-pfXabBIPa9T9pBQZr4ucNDLZNoe2Fs24Zr-NaD1cPRQjOSQkEj5B-gVTHopYBXAK2PI3TfexIMKk8yeqOvI8hUTPJD143-gvfl4KQJtZEVEr0UbQBR2U1Lb1_GuYxYb9dE9fCGhjnh1YTNtkP_GwLyZkajbA7pmdh1n-D6BKMAK-aBGm7S6vhak6LeTSMrR1NhzgqtXXN0auR8d7n5yLdRP5-mAzynPq978an_dI5euP6NvYx8cBAsNRuVc_KxqmBEwGNUwxlm00
...
**Step 2**: The User authenticates itself to the Wallet Unit. This can be done by the Wallet Instance or a Wallet Secure Cryptographic Application (WSCA). It is a prerequisite for accessing sensitive data and presenting attributes.

**Step 3**: The User selects the proximity presentation functionality.

**Step 4**: [Optional] If the initial authentication in Step 2 was not done through WSCA, a separate authentication via WSCA might be required before presenting sensitive credentials like the mdoc.
...
**Step 7**: The Relying Party Instance uses its reader to scan the QR code and retrieve the DeviceEngagement data from the mdoc.
...
**Step 10**: The RP Instance prepares a SessionEstablishment message. This message is signed by the Relying Party Instance (mdoc reader authentication as specified in [ISO18013-5#9.1.1.4]) and encrypted using the session keys derived in the previous step. The SessionEstablishment message includes the EReaderKey.Pub and a request for specific attribute(s).
...
**Step 13**: Upon receiving the SessionEstablishment message, the Wallet Unit decrypts it using the shared session key and verifies the Relying Party instance’s signature (mdoc reader authentication as specified in [ISO18013-5#9.1.1.4]) to ensure its authenticity.
...
**Step 16**: After receiving user approval, the Wallet Unit retrieves the requested mdoc Digital Credentials. It then prepares a SessionData message containing these credentials, signs it (mdoc authentication as specified in [ISO18013-5#9.1.3]). It encrypts it using the established session keys before transmitting it to the Relying Party Instance over the secure BLE channel. The signing ensures device binding and data integrity.

The mdoc response MUST be encoded in CBOR, with its structure outlined in [ISO18013-5#8.3.2.1.2.2].
