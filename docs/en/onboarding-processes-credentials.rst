...
2. The definition of the claim is established with its canonical name, its ``type``, its ``format``, its ``aliases`` and its nested structure, aligned to the :ref:`registry:Catalogue of Attributes` and without conflicts with the claims already registered.
