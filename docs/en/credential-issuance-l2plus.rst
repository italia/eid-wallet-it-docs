...
When the ``it_l2+document_proof`` object is not present in the ``authorization_details`` array, or when the Authorization Server does not support or does not accept the related hint, the PID Provider MUST authenticate the User according to its authentication policies, as defined in :ref:`credential-issuance-endpoint:User Authentication Method Selection`. In particular, the Authorization Server MUST NOT offer on a discovery page, select, or otherwise apply eID Substantial Authentication with MRTD Verification unless the request includes an ``it_l2+document_proof`` object containing the required technical parameters ``challenge_method`` and ``challenge_redirect_uri``. In the absence of a User selection via a Credential Issuer discovery page, the default authentication method MUST be CIEid LoA High.
...
Upon successful processing of the PAR, the Authorization Server redirects the User Agent to the Identity Provider for primary authentication. If the request contains a supported and accepted ``idphinting``, the Authorization Server SHOULD use it; otherwise it MUST use an Identity Provider that it has configured or selected via its own discovery page. The User completes the authentication flow and the Authorization Server correlates the authenticated identity with the active OAuth session.
