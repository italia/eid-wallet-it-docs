...
Upon successful processing of the PAR, the Authorization Server redirects the User Agent to the 1Identity Provider for primary authentication. If the request contains a supported and accepted ``idphinting``, the Authorization Server SHOULD use it; otherwise it MUST use a Identity Provider that it has configured or selected via its own discovery page. The User completes the authentication flow and the Authorization Server correlates the authenticated identity with the active OAuth session.
