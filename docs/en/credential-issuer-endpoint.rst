...
Alternatively the Credential Issuer's Metadata can be retrieved using the Credential Issuer Identifier. The Metadata document in JSON or JWT format MUST be available through the ``/.well-known/openid-credential-issuer`` endpoint as defined in Section 12.2.2 of `OpenID4VCI`_.
