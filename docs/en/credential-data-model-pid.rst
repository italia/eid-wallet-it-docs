...
.. note::
   **Identity Matching**
   ...
   In accordance with Commission Implementing Regulation `EU_2024/2977`_ and the ARF PID Rulebook, the attribute ``personal_administrative_number`` is OPTIONAL in the PID. Consequently, Relying Parties MUST NOT assume that a unique national administrative identifier is always available in the PID for the purpose of uniquely identifying the User or correlating attributes across attestations.

   Where a unique administrative identifier is absent, identity matching MUST be ensured as mandated by the EUDI Wallet Architecture Reference Framework [`EIDAS-ARF`_], in particular:

   - **ARF Topic 17 (Identity matching)**: Relying Parties providing cross-border services SHALL ensure identity matching for natural persons, employing a risk-based approach (HLR **IM_01**). Member States SHALL facilitate such matching by providing in natural-person PIDs at least four optional attributes of their choice (HLR **IM_01**). Complementary unique identification procedures MAY be required at national level where necessary (HLR **IM_03**).

   - **ARF Topic 18 / Discussion Topic K (Combined presentation of attestations)**: When attributes from a PID and/or (Q)EAAs are presented together, the Relying Party MUST be able to verify that all presented attributes refer to the same User. Pending full availability of Cryptographic Binding of Attestations (HLRs **ACP_01**–**ACP_07**), identity matching MAY be achieved through the mechanisms recognised by the EUDIW framework, including:

     - *Attribute-Based Binding*, using redundant person identification data present in the PID and in other attestations (for example ``given_name``, ``family_name`` and ``birthdate`` / ``birth_date``);
     - *Session-Based Binding*, where attributes released in a single presentation response are treated as belonging to the same User;
     - *Issuer-Attested Binding* or *Relying Party-Specific Identifiers*, where applicable;
     - *Cryptographic Binding*, proving that the private keys of the presented attestations are managed by the same WSCA/WSCD, once such a scheme is available in accordance with Topic 18.
