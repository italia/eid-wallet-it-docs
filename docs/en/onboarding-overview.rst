...
**Trust-infrastructure entities**
  ...
  - **Provider of WRPAC** and **Provider of WRPRC**: issue, respectively, the WRPAC and the WRPRC. The Provider of WRPAC SHALL log issued WRPACs according to Certificate Transparency version 2.0 (:rfc:`9162`), include at least one Signed Certificate Timestamp in each WRPAC, describe that logging in its CPS, and act as a monitor in the Certificate Transparency ecosystem when a log for access certificates is available ([`EIDAS-ARF`_] CT_01, CT_02, CT_03, CT_04). The Provider of WRPRC SHALL issue registration certificates automatically after registration, without a request from the Entity ([`EIDAS-ARF`_] RPRC_09, RPRC_13).
