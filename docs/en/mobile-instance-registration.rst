...
Mobile Appication Instance Initialization and Registration
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Since the registration flows are analogous for a Wallet or Relying Party Instance, these components are henceforth called Mobile Application Instance. Similarly, the Appication Provider backen fulfills the roles of the Wallet Provier and Relying Party backend depending on the registration process.   
...
.. figure:: ../../images/application_instance_initialization.svg
    ...
    Mobile Appication Instance Initialization Flow

**Step 1**: The User starts the Mobile Appication Instance mobile app for the first time.

**Step 2**: The Mobile Appication Instance:
...
.. note::
    ...
    **Federation Check**: The Mobile Appication Instance needs to check if the Appication Provider is part of the Federation, obtaining its protocol-specific Metadata. Non-normative examples of a response from the :ref:`Federation endpoint` with the **Entity Configuration** and the **Metadata** of the Appication Provider are presented within the :ref:`Wallet Provider Entity Configuration` and :ref:`Entity Configuration of Relying Parties` sections.

**Steps 3-5 (Challende Retrieval)**: The Mobile Appication Instance requests a one-time ``challenge`` from the :ref:`Nonce endpoint` of the Appication Provider Backend. This ``challenge`` MUST be unpredictable to serve as the main defense against replay attacks. 

Upon a successful request, the Appication Provider Backend generates and returns the ``challenge`` value to the Mobile Appication Instance. The backend MUST ensure that it is single-use and valid only within a specific time frame. 

**Step 6**: The Mobile Appication Instance, through the operating system, creates a pair of Cryptographic Hardware Keys and stores the corresponding Cryptographic Hardware Key Tag in local storage once the following requirements are met:
...
.. note::
  ...
  **WSCD**: The Mobile Appication Instance MAY use a local WSCD for cryptographic operations, including key generation, secure storage, and cryptographic processing,  on devices that support this feature. On Android devices, Strongbox is RECOMMENDED; Trusted Execution Environment (TEE) MAY be used only when Strongbox is unavailable. For iOS devices, Secure Elements (SE) MUST be used. Given that each OEM offers a distinct SDK for accessing the local WSCD, the discussion hereafter will address this topic in a general context.

  If the WSCD fails during any of these operations, for example due to hardware limitations, it will raise an error response to the Mobile Appication Instance. The Mobile Appication Instance MUST handle these errors accordingly to ensure secure operation. Details on error handling are left to the Mobile Appication Instance implementation.

**Step 7**: The Mobile Appication Instance uses the Key Attestation API, providing the "challenge" and the Cryptographic Hardware Key Tag to acquire the Key Attestation.

.. note::

  **Key Attestation API**: In this section, the Key Attestation API is assumed to be provided by device manufacturer. This service allows the verification of a key being securely stored within the device's hardware through a signed object. Additionally, it offers verifiable proof that a specific Mobile Appication Instance is authentic, unaltered, and in its original state using a specialized signed document made for this purpose.
...
If any errors occur in any Key Attestation APIS process, such as device integrity verification, for example, due to unavailable Key Attestation APIs, an internal error, or an invalid challenge in the integrity request, the Key Attestation APIS raise an error response. The Mobile Appication Instance MUST process these errors accordingly. Details on error handling are left to the Mobile Appication Instance implementation.
...
**Step 9 (Mobile Appication Instance Registration Request)**: The Mobile Appication Instance sends a request to the :ref:`Mobile Appication Instance Registration Request` of the Appication Provider Backend to register the Mobile Appication Instance, identified by the Cryptographic Hardware Key public key. 
...
.. note::
  ...
  As seen in the previous steps, the Key Attestation API creates a Key Attestation linked to the provided "challenge" and the public key of the Wallet Hardware. This process eliminates the need to send the Wallet Hardware public key directly, as it is already included in the key attestation. The ``hardware_key_tag`` serves as a reference or identifier for the corresponding Cryptographic Hardware key stored by the Appication Provider. Therefore, the Appication Provider can associate the received ``hardware_key_tag`` with the appropriate Cryptographic Hardware key in its storage.

.. warning::
  During the registration phase of the Mobile Appication Instance with the Appication Provider it is also necessary to associate the Wallet Instace with a specific User, authenticating the User with the Appication Provider. The authentication mechanism is at the discretion of the Appication Provider and it will not be addressed within these guidelines, as each Appication Provider may have its User authentication systems already implemented.


**Steps 10-12 (Mobile Appication Instance Registration Response)**: The Appication Provider validates the ``challenge`` and ``key_attestation`` signature, therefore:

  1. It MUST verify that the ``challenge`` was generated by Appication Provider and has not already been used.
  2. It MUST validate the ``key_attestation`` as defined by the device manufacturers' guidelines.
  3. It MUST verify that the device in use has no security flaws and reflects the minimum security requirements defined by the Appication Provider.
  4. If these checks are passed, it MUST register the Mobile Appication Instance, keeping the Cryptographic Hardware Key Tag (hardware_key_tag), the Public Hardware Key (hardware_key_pub) and possibly other useful information related to the device.
  5. It SHOULD associate the Mobile Appication Instance with a specific User uniquely identified within the Appication Provider's systems. This will be useful for the lifecycle of the Mobile Appication Instance and for a future revocation.

Upon successful registration of the Mobile Appication Instance, the Appication Provider responds with a confirmation of success.

**Steps 13-14**: The Mobile Appication Instance has been initialized and becomes operational.
...
The request for a challenge MUST be an HTTP GET request sent to the Appication Provider’s Nonce Endpoint.
...
To register a Mobile Appication Instance, the request to the Appication Provider MUST use the HTTP POST method with ``Content-Type`` set to `application/json`. The request body MUST contain the following claims:
...
.. list-table:: 
    ...
    * - **Claim**
      ...
      - **Reference**
    * - **challenge**
      - MUST be set to the value obtained from the Appication Provider through the Nonce Endpoint.
      - This specification.
    * - **hardware_key_tag**
      - It MUST be set with the unique identifier of the **Cryptographic Hardware Keys** and encoded in base64url.
      - This specification.
    * - **key_attestation**
      - It MUST be an attestation which guarantees the secure generation, storage and usage of the key pair generated by the Mobile Appication Instance. This can be an array containing a certificate chain whose leaf certificate is the Key Attestation obtained from the device **Key Attestation APIs**, signed with the device hardware key.
      - This specification.

Below is a non-normative example of a Mobile Appication Instance Registration Request.
...
Mobile Appication Instance Registration Response
......................................................

If a Mobile Appication Instance Registration Request is successfully validated, the Appication Provider provides an HTTP Response with status code 204 (No Content).

Below is a non-normative example of  a Mobile Appication Instance Registration Response.
...
If any errors occur during the Mobile Appication Instance registration, an error response MUST be returned. Refer to :ref:`Error Handling for Wallet Instance Management` and  for details on error codes and descriptions.
