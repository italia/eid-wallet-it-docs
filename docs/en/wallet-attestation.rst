...
**Steps 8-10**: The Wallet Instance:
  ...
  *  produces an hardware_signature by signing the ``client_data_hash`` with the Wallet Hardware's private key, serving as a proof of possession for the Cryptographic Hardware Keys.
...
**Steps 13-17**: The Wallet Provider Backend evaluates the Wallet Attestation Request and MUST perform the following checks:
    ...
    5. The ``client_data`` MUST be reconstructed using the ``challenge`` and the ``jwk`` public key.  The ``hardware_signature`` is then validated using the registered Cryptographic Hardware Key's public key associated with the Wallet Instance.
