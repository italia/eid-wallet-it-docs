...
In order to implement and provide Technical Solutions, a Relying Party could take advantage of services provided by an Intermediary Entity. A **Relying Party Intermediary** is an Organization Entity that may act on behalf of the Relying Party offering services to it, making the Relying Party able to connect to Wallet Instance and authenticate Users and verify User's Digital Credentials. In the **European Digital Identity** legal framework (`EU_2024_1183`_), intermediaries acting on behalf of relying parties are treated as relying parties for registration and authentication towards European Digital Identity Wallets and **shall not store data about the content of the transaction** between the Wallet user and the intermediated Relying Party (Article 5b(8)). Minimum registration information, public registries in human- and machine-readable form, and common authentication mechanisms for wallet-relying parties are specified or completed at Union level through implementing acts. The EUDI Wallet Architecture and Reference Framework (`ARF`_) discusses high-level requirements for relying-party registration—including intermediaries—in *Topic X – Relying Party registration* (`ARF_TOPIC_X_RP`_). These Technical Specifications define how Relying Parties and Relying Party Intermediaries onboard, publish federation metadata, and operate Relying Party Solutions in line with national measures while remaining interoperable with that framework.

.. note::
  The distinction between a **Federation Intermediary** (an entity that technically facilitates participation in the OpenID Federation as defined in `OID-FED`_) and a **Relying Party Intermediary** (an entity that manages verification services on behalf of Relying Parties under the eIDAS2 Regulation) is operationally relevant: the Trust Anchor registers the Relying Party Intermediary with a Trust Mark containing the ``entity_type: "intermediary"`` claim, which the Wallet Instance uses to identify and display to the User that the requesting Relying Party operates through a recognized Intermediary. See :ref:`trust-infrastructure:Trust Marks for Relying Party Intermediaries`.

A Relying Party provides at least one of the following components:

- **Relying Party Backend**: It handles Relying Party Instances registration and the relative Certificate management. It obtains X.509 Certificate according to the :ref:`trust-infrastructure:The Infrastructure of Trust`. It SHOULD also provide a X.509 Certificate to its Relying Party Instances. It also may implement additional web services and business logic for its own purposes and use cases.
