...
.. note::
  In the implementation profile described by these technical specifications, a Relying Party Intermediary is also an OpenID Federation Intermediate Entity (**IT-Wallet Intermediary**): it publishes its own Entity Configuration and issues Subordinate Statements and Trust Marks for its affiliated Relying Parties. The Trust Anchor registers it with a ``trust_mark_type`` ending with ``rp_intermediary``, which the Wallet Instance uses to identify and display to the User that the requesting Relying Party operates through a recognized Intermediary. See :ref:`trust-infrastructure:Trust Marks for Relying Party Intermediaries`.
