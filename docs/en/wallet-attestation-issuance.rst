...
**Steps 23-28**: The Wallet Provider Backend evaluates the Wallet App and Wallet unit Attestation Request and MUST perform the following checks (:ref:`WP_143 <wallet-instance-optional-testcases>`):
  ...
  9. The Wallet Provider MUST determine the WSCD security level of the Wallet Unit (WL2 or WL3) based on the device's Cryptographic Hardware capabilities and MUST encode this information in the Wallet Unit Attestation, as specified in :ref:`wallet-solution-requirements:wscd-security-levels`. The security level assessment MUST consider the properties of the Cryptographic Hardware (SE, TEE, or hybrid) and the key management capabilities available on the device.
