...
.. note::
    ...
    Regardless of the User authentication method (CIEid LoA High or eID Substantial with MRTD Verification), the PID Provider MUST verify that the Wallet Unit's WSCD security level is WL3 before issuing a PID, in accordance with :ref:`wallet-solution-requirements:wscd-security-levels`.
