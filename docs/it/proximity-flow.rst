...
- *Autenticazione dell'App di Verifica* seguendo i meccanismi definiti nell'`ISO18013-5`_ per l'*autenticazione del lettore*.
- Meccanismo di *Recupero del Dispositivo* basato su Bluetooth Low Energy (BLE) per la sottofase di comunicazione. Il meccanismo di *Server Recovery* NON DEVE essere supportato.
- *docType* e *Namespace* domestico definiti in questa specifica tecnica sono in aggiunta a quelli già definiti nell'`ISO18013-5`_ per l'mDL (vedi :ref:`credential-data-model:Attestato Elettronico in formato mdoc-CBOR` per maggiori dettagli).
- *Validazione dell'Istanza del Wallet* attraverso l'Attestato di Unità di Wallet.
...
Di seguito è riportato un esempio non normativo di messaggio un ``SessionEstablishment`` codificato in CBOR che contiene la richiesta mdoc di un Attestato di Wallet insieme a una Attributo Elettronico mDL.
...
Di seguito è riportato un esempio non normativo di un ``SessionData`` codificato in CBOR che contiene la risposta mdoc di un Attestato di Unità di Wallet e un mDL.
...
.. note::
  Durante la presentazione di prossimità, l'Istanza del Wallet potrebbe non essere in grado di recuperare un Attestato di Unità di Wallet aggiornato; in questo caso, l'Istanza del Wallet DOVREBBE inviare l'ultima versione dell'Attestato di Unità di Wallet. È demandato alla Relying Party determinare se una presentazione con un Attestato di Wallet valido ma scaduto costituisca o meno un ostacolo al flusso di presentazione valutando, caso per caso, le implicazioni.
