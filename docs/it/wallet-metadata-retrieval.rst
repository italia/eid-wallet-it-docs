...
In primo luogo, con il list endpoint (GET /list) ottengono l'``entity_id`` di tutti i subordinati del Trust Anchor. Per ottenere solo l'``entity_id`` relativi ai Wallet Provider, è POSSIBILE utilizzare il parametro di query ``entity_type``. 
