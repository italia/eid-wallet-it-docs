...
Per implementare e fornire Soluzioni Tecniche, una Relying Party potrebbe avvalersi dei servizi forniti da un Intermediario. Un **Intermediario di Relying Party** è un'Entità Organizzativa che può agire per conto della Relying Party offrendo servizi ad essa, consentendo alla Relying Party di connettersi all'Istanza del Wallet e autenticare gli Utenti e verificare gli Attestati Elettronici dell'Utente. Nel quadro giuridico dell'**identità digitale europea** (`EU_2024_1183`_), gli intermediari che agiscono per conto delle relying party sono equiparati alle relying party ai fini della registrazione e dell'autenticazione verso i Portafogli di identità digitale europea e **non devono conservare dati sul contenuto della transazione** tra l'utente del Portafoglio e la Relying Party intermediata (articolo 5b, paragrafo 8). Informazioni minime di registrazione, registri pubblici in forma leggibile dall'uomo e dalla macchina, e meccanismi comuni di autenticazione delle wallet-relying party sono definiti o completati a livello di Unione mediante atti di esecuzione. L'Architecture and Reference Framework del Portafoglio EUDI (`ARF`_) raccoglie requisiti di alto livello sulla registrazione delle relying party—inclusi gli intermediari—nel *Topic X – Relying Party registration* (`ARF_TOPIC_X_RP`_). Le presenti Specifiche Tecniche definiscono come Relying Party e Intermediari di Relying Party effettuano l'onboarding, pubblicano i metadati di federazione e operano le Soluzioni di Relying Party in coerenza con le misure nazionali e con tale quadro.

.. note::
  La distinzione tra **Intermediario di Federazione** (entità che facilita tecnicamente la partecipazione alla Federazione OpenID come definito in `OID-FED`_) e **Intermediario di Relying Party** (entità che gestisce servizi di verifica per conto di Relying Party ai sensi del Regolamento eIDAS2) è operativamente rilevante: il Trust Anchor registra l'Intermediario di Relying Party con un particolare ``trust_mark_type`` che termina con ``"intermediary"``, che l'Istanza del Wallet utilizza per identificare e mostrare all'Utente che il Relying Party richiedente opera tramite un Intermediario riconosciuto. Vedi :ref:`trust-infrastructure:Trust Mark degli Intermediari di Relying Party`.
