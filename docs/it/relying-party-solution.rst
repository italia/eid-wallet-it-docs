...
.. note::
  Nel profilo implementativo descritto dalle presenti specifiche tecniche, un Intermediario di Relying Party è anche un'Entità Intermedia OpenID Federation (**Intermediario IT-Wallet**): pubblica la propria Entity Configuration ed emette Subordinate Statement e Trust Mark per le Relying Party affiliate. Il Trust Anchor lo registra con un ``trust_mark_type`` che termina con ``rp_intermediary``, che l'Istanza del Wallet utilizza per identificare e mostrare all'Utente che la Relying Party richiedente opera tramite un Intermediario riconosciuto. Vedi :ref:`trust-infrastructure:Trust Mark degli Intermediari di Relying Party`.
