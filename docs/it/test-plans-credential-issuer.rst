...
.. list-table::
  ...
  * - CI_015b
    - Emissione, Sicurezza
    - Recupero della Chiave Pubblica dalla Wallet Instance Attestation (WIA) nella PAR
    - Il Credential Issuer recupera correttamente la chiave pubblica dal claim cnf.jwk della Wallet Instance Attestation (WIA).
  * - CI_015c
    - Emissione, Sicurezza
  ...
  * - CI_029
    ...
    - Il Credential Issuer verifica con successo l’affidabilità e l’appartenenza indiretta alla Federazione dell’istanza del Wallet attraverso una validazione completa della Wallet Instance Attestation (WIA).
  * - CI_030
    - Emissione, Fiducia
    - Validazione dell’appartenenza alla Federazione del Wallet Provider
    - Il Credential Issuer conferma che il Wallet Provider (emittente della Wallet Instance Attestation (WIA)) è un membro della Federazione riconosciuto e affidabile controllando i registri e le liste di fiducia della Federazione.
  * - CI_031
    - Emissione, Sicurezza
    - Validazione della firma crittografica della Wallet Instance Attestation (WIA)
    - Il Credential Issuer valida con successo la firma crittografica della Wallet Instance Attestation (WIA) utilizzando la chiave pubblica del Wallet Provider, garantendo integrità e autenticità della firma.
  * - CI_032
    - Emissione, Sicurezza
    - Controllo della scadenza della Wallet Instance Attestation (WIA)
    - Il Credential Issuer verifica che la Wallet Instance Attestation (WIA) non sia scaduta al momento della verifica confrontando i timestamp dichiarati con l’orario corrente.
  * - CI_033
    - Emissione, Sicurezza
    - Accettazione delle chiavi crittografiche attestate nella Wallet Instance Attestation (WIA)
  ...
  * - CI_034
    - Emissione, Sicurezza
    - Verifica della sicurezza e conformità del dispositivo tramite Wallet Instance Attestation (WIA)
    - Il Credential Issuer si affida alle dichiarazioni della Wallet Instance Attestation (WIA) per confermare che l’istanza del Wallet operi su un dispositivo sicuro e affidabile che soddisfi gli standard di sicurezza richiesti dall’Issuer.
  * - CI_035
    - Emissione, Fiducia
    - Valutazione della catena di fiducia del Wallet Provider
    - Il Credential Issuer valuta con successo l’intera catena di fiducia dell’emittente della Wallet Instance Attestation (WIA) (Wallet Provider).
  * - CI_036
  ...
  * - CI_051
    - Emissione, Sicurezza
    - Autenticazione CieID High per il PID
    - Il PID Provider esegue con successo l'autenticazione dell'utente basata sullo schema CieID con LoA High (CIE L3). Questo caso si applica al PID, non all'IT-Wallet ID.
  * - CI_052
    - Emissione, Sicurezza e Privacy
  ...
  * - CI_055
    ...
    - Il (Q)EAA Provider utilizza il protocollo OpenID4VP per richiedere la presentazione del PID o dell'IT-Wallet ID dall'istanza del Wallet.
  * - CI_056
  ...
  * - CI_196
    - Modello di Dati e ciclo di vita, Interoperabilità
    - Attributi utente dell'IT-Wallet ID
    - L'IT-Wallet ID è emesso con gli attributi utente definiti in :ref:`credential-data-model-it-wallet-id:Modello di Dati dell'IT-Wallet ID`, distinti dall'insieme di attributi del PID.
  * - CI_197
    - Emissione, Autenticazione
    - Autenticazione primaria per IT-Wallet ID
    - L'EAA Provider di IT-Wallet ID autentica l'Utente con CieID LoA High (CIE L3) come metodo primario.
  * - CI_198
    - Emissione, Autenticazione
    - Autenticazione eID Substantial con Verifica MRTD
    - Quando non si usa il PIN CieID, l'EAA Provider di IT-Wallet ID supporta il flusso di Autenticazione eID Substantial con Verifica MRTD in :ref:`credential-issuance-l2plus:Autenticazione eID Substantial con Verifica MRTD per Emissione IT-Wallet ID`. Gli attori di quel flusso conservano i log secondo le General Log Retention Policies.
  * - CI_199
    - Emissione, Autenticazione
    - Authorization Details L2+
    - Per l'emissione di IT-Wallet ID con Verifica MRTD, l'array ``authorization_details`` della PAR contiene un oggetto con ``type`` ``it_l2+document_proof`` e i claim richiesti. Se quell'oggetto è assente, l'autenticazione torna a CieID LoA High.
  * - CI_200
    - Emissione, Autenticazione, Sicurezza
    - Correlazione identità IdP–MRTD
    - L'EAA Provider lega l'identità verificata dall'IdP (CieID/SPID Substantial) con l'identità nell'MRTD confrontando almeno il codice fiscale o un identificativo univoco equivalente presente in entrambe le fonti. Una discordanza interrompe l'emissione. L'autenticazione primaria LoA High salta l'MRTD PoP.
  * - CI_201
    - Presentazione, Sicurezza
    - Identity matching con identificativo nazionale univoco
    - Quando è presentato ``personal_administrative_number`` o ``tax_id_code``, la Relying Party lo usa come chiave primaria di identity matching prima di qualsiasi riconciliazione con un record Utente memorizzato.
  * - CI_202
    - Presentazione, Sicurezza
    - Attribute-Based Binding senza identificativo univoco
    - Quando un PID è presentato senza ``personal_administrative_number``, l'Attribute-Based Binding non si limita a una coppia di nomi. La Relying Party confronta almeno ``family_name``, ``given_name`` e ``birth_date`` (o ``birthdate``) dopo normalizzazione, e NON DEVE completare il matching su un sottoinsieme che consentirebbe uno scambio di persona. Binding di sessione, attested dall'Issuer o crittografico POSSONO integrare il confronto.
  * - CI_203
    - Emissione, Privacy
  ...
  * - CI_204
    - Emissione, Interoperabilità
    - WIA in emissione
    - In emissione il Credential Issuer valida una Wallet Instance Attestation (WIA), non una «Wallet Attestation» legacy. La WUA, ove usata, è un manufatto distinto.
  * - CI_205
    - Emissione, Autenticazione
