...
.. list-table::
   ...
   * - **verification**
     - *(map, OPZIONALE)*. Identificativo del dato format-encoded `verification` come definito nella Sezione :ref:`credential-data-model:Attributi di Metadati Format-Agnostic dell'Attestato Elettronico`. La CBOR map include i seguenti membri:
         ...
         * ``assurance_level`` *(tstr, OBBLIGATORIO)*: livello di garanzia dell'identità garantito durante l'autenticazione dell'Utente. Il valore DEVE corrispondere a uno dei valori mappati nell'array ``acr_values_supported`` presente nel :ref:`credential-issuer-entity-configuration:Metadata del Fornitore di Attestati Elettronici`.
