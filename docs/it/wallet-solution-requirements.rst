...
- L'istanza del Wallet DEVE stabilire un rapporto di fiducia con gli altri partecipanti dell'ecosistema del Wallet, come gli Emittenti di Credenziali e le Relying Parties. Nel caso degli Emittenti di Credenziali, l'istanza del Wallet presenta sia la Wallet App Attestation che la Wallet Unit Attestation, mentre per le Relying Parties presenta solo la Wallet App Attestation.
...
La Wallet Unit Attestation contiene informazioni che garantiscono che le chiavi utilizzate per il collegamento crittografico delle Credenziali Digitali siano archiviate in un WSCD **affidabile**.
Inoltre, fornisce un metodo per autenticare il WSCD presso l'Emittente di Credenziali e verifica che l'unità del Wallet non sia stata revocata.

I requisiti per la Wallet Unit Attestation sono definiti di seguito:

- La Wallet Unit Attestation DEVE fornire al PID Provider o all'Attestation Provider informazioni sulle capacità del WSCA e del WSCD dell'unità del Wallet, in modo che possano prendere una decisione ben fondata sull'opportunità di emettere un PID o un'attestazione per tale unità del Wallet.
- La Wallet Unit Attestation DEVE consentire ai PID Provider e agli Attestation Provider di verificare l'autenticità e lo stato di revoca dell'unità del Wallet.
- Durante l'emissione di un PID o di un'attestazione vincolata al dispositivo, un'unità del Wallet DEVE recuperare, dai metadati dell'emittente (come specificato in OpenID4VCI_), i requisiti del PID Provider o dell'Attestation Provider riguardanti l'autenticazione dell'utente e l'archiviazione delle chiavi da parte del WSCA/WSCD. L'unità del Wallet DEVE determinare quale dei propri WSCA/WSCD, se presente, soddisfi tali requisiti. Se un WSCA/WSCD conforme è disponibile per l'unità del Wallet, quest'ultima DEVE richiederne la generazione di una nuova coppia di chiavi per il nuovo PID o l'attestazione. L'unità del Wallet DEVE fornire al PID Provider o all'Attestation Provider la Wallet Unit Attestation che descrive le proprietà del WSCA/WSCD che ha generato la nuova chiave privata del PID o dell'attestazione.
- Se un'unità del Wallet contiene più WSCA, essa DEVE, in modo interno e sicuro, tenere traccia di quali PID e attestazioni sono associati a ciascun WSCA.
- Un'unità del Wallet DEVE presentare una Wallet Unit Attestation solo come parte del processo di emissione di un PID o di un'attestazione.
- La Wallet Unit Attestation DEVE consentire ai PID Provider di richiedere a un Wallet Provider la revoca di un'unità del Wallet, includendo un identificatore per l'unità del Wallet all'interno della WUA (ad esempio, un URI e un indice a una Attestation Status List). Il Wallet Provider DEVE garantire che tale identificatore dell'unità del Wallet non consenta il tracciamento dell'utente.
