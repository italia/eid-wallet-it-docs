...
Quando l'oggetto ``it_l2+document_proof`` non è presente nell'array ``authorization_details``, oppure quando l'*Authorization Server* non supporta o non accetta il relativo *hint*, il Provider PID DEVE autenticare l'Utente secondo le proprie politiche di autenticazione, come definito in :ref:`credential-issuance-endpoint:Selezione del Metodo di Autenticazione dell'Utente`.La Risposta PAR e la Richiesta di Autorizzazione sono le stesse delle Specifiche IT-Wallet.
