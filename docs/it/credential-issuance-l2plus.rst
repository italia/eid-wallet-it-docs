...
Quando l'oggetto ``it_l2+document_proof`` non è presente nell'array ``authorization_details``, oppure quando l'*Authorization Server* non supporta o non accetta il relativo *hint*, il Provider PID DEVE autenticare l'Utente secondo le proprie politiche di autenticazione, come definito in :ref:`credential-issuance-endpoint:Selezione del Metodo di Autenticazione dell'Utente`. In particolare, l'*Authorization Server* NON DEVE offrire in una pagina di *discovery*, selezionare o altrimenti applicare l'Autenticazione eID Substantial con Verifica MRTD a meno che la richiesta includa un oggetto ``it_l2+document_proof`` contenente i parametri tecnici obbligatori ``challenge_method`` e ``challenge_redirect_uri``. In assenza di selezione dell'Utente tramite pagina di *discovery* del Credential Issuer, il metodo di autenticazione di *default* DEVE essere CieID con Livello di Garanzia Alto. La Risposta PAR e la Richiesta di Autorizzazione sono le stesse delle Specifiche IT-Wallet.

Fase 2: Autenticazione Primaria
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Dopo l'elaborazione con successo del PAR, il Server di Autorizzazione reindirizza l'User Agent al Provider di Identità per l'autenticazione primaria. Se la richiesta contiene un ``idphinting`` supportato e accettato, il Server di Autorizzazione DOVREBBE utilizzarlo; altrimenti DEVE utilizzare un Provider di Identità da esso configurato o selezionato tramite propria pagina di *discovery*. L'Utente completa il flusso di autenticazione e il Server di Autorizzazione correla l'identità autenticata con la sessione OAuth attiva.
