...
1. Recuperare il proprio Subordinate Statement dal superiore immediato ogni 24 ore utilizzando il fetch endpoint definito in :ref:`trust-infrastructure:Endpoint API di Federazione`. Ciò DOVREBBE essere fatto entro un intervallo di tempo non superiore a 24 ore.
...
1. **Revoca della Chiave**: Il superiore pubblica la Certificate Revocation List (CRL) o la voce Historical Keys (HK) revocando la vecchia chiave come definito in :ref:`trust-infrastructure:Revoca di Certificati X.509`.
